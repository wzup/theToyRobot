...
var os = require("os"), // to have platform independent EOL
    ...
    robot = new(require('./app/robot'))(), // a robot instance
    EOL = os.EOL,
...
function doAction(sCommand) {
    ...
    if (sCommand.match(/place/i)) {
        ...
        if (res) {
            // stdout.write([result, EOL].join(''));
            return res;
        }
        // stdout.write([robot.report(), EOL].join('')); // удалить в продакшене
        return [robot.report(), "  --УДАЛИЬ В ПРОДЕ--  ", EOL].join('');
    } else if (sCommand.match(/move/i)) {

    } else if (sCommand.match(/left/i)) {} else if (sCommand.match(/right/i)) {
